...
public class SudokuGrid : MonoBehaviour
{
    ...
    void Start()
    {
        if (grid_square.GetComponent<GridSquare>() == null)
            Debug.LogError("Este objeto de juego debe tener adjunto un script de GridSquare!");
    ...
    private void SpawnGridSquares()
    {
        ...
        for (int row = 0; row < rows; row++)
        {
            for (int column = 0; column < columns; column++)
            {
                ...
                grid_squares_[grid_squares_.Count - 1].transform.parent = this.transform; // crea una instancia de este objeto de juego como un elemento secundario del objeto que contiene este script.
